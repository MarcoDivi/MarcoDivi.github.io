...
export class MovieInfoComponent {
  ...
  // navigazione da home.component -> app.routes -> Resolver per recuperare i dati -> Qui
  constructor(private route: ActivatedRoute, private dialog: MatDialog){
    route.data.subscribe({
      next: (routeData) => {
        let resolvedMovie: ResolvedMovie = routeData['movieInfo'];
        this.movie = resolvedMovie.info;
        this.trailer = resolvedMovie.trailer.results.find(video => video.site == "YouTube" && video.type == "Trailer" && video.official == true && !video.name.toLowerCase().includes("teaser"));
      }
    })
  }
